...
export class RoleController {
    ...
    @Get()
    async getPaginated(@Query() queryParams: ListCriteria, @Res() res: FastifyReply) {
        const results: Paginated<Role> = await this.roleService.getPaginated(queryParams);
        HttpResponseService.sendSuccess<Paginated<Role>>(res, HttpStatus.OK, results);
    }
    ...
    @Patch(':id')
    async update(@Param('id') id: string, @Body() updateRoleDto: UpdateRoleDto, @Res() res: FastifyReply) {
    ...
    @Post('suggestions')
    async requestSuggestions(@Body() search: string, @Req() req: FastifyRequest, @Res() res: FastifyReply) {
